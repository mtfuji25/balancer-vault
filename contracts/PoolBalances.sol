...
interface IPoolBalances {
    ...
    function toJoinPoolBalanceChange(IVault.JoinPoolRequest memory request)
        external
        pure
        returns (PoolBalanceChange memory);

    function toExitPoolBalanceChange(IVault.ExitPoolRequest memory request)
        external
        pure
        returns (PoolBalanceChange memory);
}

abstract contract PoolBalances is IPoolBalances, Fees, ReentrancyGuard, PoolTokens, UserBalance {
    ...
    function toJoinPoolBalanceChange(JoinPoolRequest memory request)
        external
        override
        pure
        returns (PoolBalanceChange memory change)
    {
        return _toPoolBalanceChange(request);
    }
     ...
     */
    function toExitPoolBalanceChange(ExitPoolRequest memory request)
        external
        override
        pure
        returns (PoolBalanceChange memory change)
    ...
    function _callJoinPoolBalanceChange(
        bytes32 poolId,
        address sender,
        address payable recipient,
        PoolBalanceChange memory change,
        ...
        )
    {
        ...
        ) = _callPoolBalanceChange(PoolBalanceChangeKind.JOIN, poolId, sender, recipient, change, balances);
    }
